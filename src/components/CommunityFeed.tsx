...
export const CommunityFeed: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const fetched: Post[] = await response.json();
      if (fetched.length === 0) {
      ...
      console.error("MongoDB read failed, falling back to seed data:", e);
      setPosts(SEED_COMMUNITY_POSTS);
    } finally {
      setLoading(false);
    }
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      ...
      console.warn("Could not sync like to MongoDB:", e);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const authorName = authUser?.displayName || user?.name || 'Sustainable Guardian';
    const authorEmail = authUser?.email || 'guest@earth.org';
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to create post in MongoDB");
      }
      ...
      console.error("Error creating post on MongoDB:", err);
