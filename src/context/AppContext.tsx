...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchUserData = async (uid: string) => {
    try {
      ...
      console.error("Error reading MongoDB profile: ", e);
    } finally {
  ...
  // Debounce timer ref for cloud sync — avoids hammering MongoDB on rapid state changes
  const syncDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Sync only essential user data, logs, and goals to MongoDB.
  ...
  const syncDataToCloud = useCallback(async (
    ...
    if (authUser && activeU) {
      try {
        ...
        console.error('[carbon-compass] MongoDB sync failed:', e);
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, activityLogs, goals, footprint, authUser]);
  ...
  // Debounced MongoDB cloud sync — fires 800ms after last change
  // Keeps authUser OUT of deps to avoid syncing on every auth event
  useEffect(() => {
